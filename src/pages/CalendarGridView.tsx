...
const CalendarGridView: React.FC<CalendarGridViewProps> = ({ activeDate, onDateClick, onActiveDateChange }) => {
    const { monthGroups, loadMore, handleDeleteEvent, getEventsForDate } = useCalendarGridStore(activeDate);
            ...
            {monthGroups.map((monthGroup) => (
                <div key={monthGroup.monthKey} className="mb-0">
                    ...
                    {monthGroup.weeks.map((weekDays, weekIdx) => {
                        ...
                        return (
                            <div key={`${monthGroup.monthKey}-${weekIdx}`} className="mb-0">
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <span className={`text-3xl font-black font-serif-art text-gray-900 dark:text-white`}>{dayjs(date).date()}</span>
                                                </div>
